...
file_path = "../output/perf_instructions.csv"
...
for i, dataset in enumerate(datasets):
    ...
    for search_method in search_methods:
        if search_method in dataset_data['search_type'].unique():
            ...
            avg_data = method_data.groupby('density')['llc_miss'].mean()  # ns를 s로 변환하여 평균값 계산
    ...
    ax.set_ylabel('instructions', fontsize=fffsize, labelpad=0)
...
# PDF 파일로 저장
plt.savefig(output_dir + 'alex_instructions.png', bbox_inches='tight')
